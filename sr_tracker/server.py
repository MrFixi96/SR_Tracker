...
#instancelist = [SReq(app.config[FIELD_NAMES]) for r in range(size)]
instancelist = []

FIELD_NAMES = ('Oracle Service Request #', "Site Name: Site Name", "Site Name: Site ID", "Severity",
...
# ************************
# URL: /uploads/
# Purpose: Allow user to upload a csv file and parse each request into an object for further use
# ************************
...
def upload():
    if request.method == 'POST':
        ...
        # Get file from http request object and save it to the os filesystem temp folder
        ...
        reader = csv.DictReader((line.replace('\0', '') for line in csvfile), FIELD_NAMES)
        ...
        # Process the rows and set the object attributes (skipping the header row)
        i = 0
        skip = 0
        if reader:
            for row in reader:
                ...
                for field, value in row.items():
                    ...
                    # If field == row then this is a header row, skip it
                    if field == value:
                        skip = 1
                        ##### TODO insert code to delete extra object
                        continue

                    instancelist.append(SReq(FIELD_NAMES))
...
def download():
    ...
    def generator():
        ...
        for each in instancelist:
            ...
            output.append(each.output_json_string())
            if DEBUG_PRINT:
                each.print_vals()
                print(str(each.output_json_string()))
